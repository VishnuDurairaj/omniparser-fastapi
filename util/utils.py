...
# ----------------------------
# OCR Initialization - GPU Enabled
# ----------------------------
# Only initialize OCR readers once, with GPU support
_ocr_initialized = False
reader = None
paddle_ocr = None

def _init_ocr():
    """Initialize OCR readers lazily with GPU support"""
    global _ocr_initialized, reader, paddle_ocr
    if _ocr_initialized:
        return
    ...
    paddle_ocr = PaddleOCR(
        lang='en',
        use_angle_cls=False,
        use_gpu=use_gpu,  # CRITICAL FIX: Enable GPU for PaddleOCR
        show_log=False,
        max_batch_size=1024,
        use_dilation=True,
        det_db_score_mode='slow',
        rec_batch_num=1024,
        enable_mkldnn=not use_gpu,  # Disable MKL-DNN when using GPU
        use_tensorrt=False,  # Set to True if you have TensorRT installed
    )
    
    _ocr_initialized = True
    print(f"OCR initialized with GPU support: {use_gpu}")
...
def get_parsed_content_icon(filtered_boxes, starting_idx, image_source, caption_model_processor, prompt=None, batch_size=128):
    """GPU-optimized icon content parsing"""
    to_pil = ToPILImage()
    ...
    if use_local_semantics:
        ...
        if 'phi3_v' in caption_model.config.model_type:
            ...
            parsed_content_icon = get_parsed_content_icon(
                filtered_boxes, starting_idx, image_source, caption_model_processor, 
